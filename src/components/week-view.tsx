...
import type { Kind, Week } from "@/lib/updates";
import { shortRange } from "@/lib/updates";
      ...
      <div className="space-y-6 px-4 pt-8 pb-2 sm:px-8">
        {week.sections.map((section, index) => (
          <section key={section.title} className="relative">
            ...
            <div className="rounded-2xl bg-white px-5 pt-7 pb-6 shadow-[0_2px_14px_rgba(47,52,57,0.07)] sm:px-7">
        ...
        {week.stat && (
          <div className="rounded-2xl bg-kd-ink px-6 py-8 text-center">
            <p className="text-5xl font-extrabold text-kd-lime">{week.stat.value}</p>
            <p className="mt-3 text-[15px] leading-relaxed font-medium text-white/85">
              <RichText>{week.stat.text}</RichText>
            </p>
          </div>
        )}
      ...
      <nav className="no-print flex items-stretch gap-3 px-4 py-8 sm:px-8">
        {older ? (
          ...
          >
            ...
            <span className="mt-0.5 block text-sm font-bold text-kd-ink">
              {shortRange(older.start, older.end)}
            </span>
          </Link>
        ) : (
        ...
        {newer ? (
          ...
          >
            ...
            <span className="mt-0.5 block text-sm font-bold text-kd-ink">
              {shortRange(newer.start, newer.end)}
            </span>
          </Link>
        ) : (
          <span className="flex-1" />
